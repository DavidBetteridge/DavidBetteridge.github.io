...
questions = {
  ...
  "A8888": {
    ...
    evalulator: (answer) => answer.includes("7.75"),
    next: "B324"
  },

  "B324": {
    text: "Down the creepy alley how many flats are in the Old Mill?",
    evalulator: (answer) => answer.includes("4"),
    next: "MS123"
  },
  ...
  "LL3333": {
    ...
    evalulator: (answer) => parseInt(answer, 10) >= 18 && parseInt(answer, 10) <= 22,
    next: "W66455"
  },

   "W66455": {
    text: "Go past the Perky Peacock which Womble is having an argument?",
    evalulator: (answer) => answer.toLowerCase().includes("wellington"),
    next: "W333"
  },

  "W333": {
    text: "Who is underneath Craven W.V on the War Memorial?",
    evalulator: (answer) => answer.toLowerCase().includes("crawford"),
    next: "FFFF"
  },  
...
const checkAnswer = (event) => {
  ...
  if (getQuestion().evalulator(answer)) {
